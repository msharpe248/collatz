...
Two remarks on reliability. First, everything rests on
...
Second, the verification procedure is small (under a hundred lines,
independent of the search code) and is itself a candidate for
formalization: the certificate check is a finite integer computation
of precisely the kind proof assistants discharge by reflection, and
formalizing Theorem~\ref{thm:kl} --- an induction over difference
inequalities --- would make the exponent the first machine-verified
density bound for the $3x+1$ problem. The companion repository
contains a Lean~4 formalization of related results (Terras' theorem in
finite form among them) where the required infrastructure already
exists \cite{companion}.

\subsection*{Reproducibility}
The search engine, the one-shot driver, the exact verifier, and
instructions are in the repository accompanying this paper
(\code{analysis/kl\_exponent.py}, \code{kl\_oneshot.py},
\code{kl\_certificate.py}); all runs are deterministic. Finding the
$k = 17$ certificate takes under an hour on a laptop; verifying it
exactly takes minutes.
