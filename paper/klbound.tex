...
$\gamma = 0.8476\ldots > 0.84$ at level $k = 12$, $\gamma =
0.8569\ldots$ at $k = 13$ and $\gamma = 50\log_2(101210/100000) =
0.8676\ldots$ at $k = 14$ ($1{,}594{,}323$ classes), the first
...
the kernel has not yet been asked to check. The $k = 13$ and $k = 14$ instances have also been run: \code{K13.density\_bound} (files \code{KL13Data},
\code{KL13Check0--2}, \code{KL13}; $531{,}441$ classes in $243$ shards,
$2$\,h\,$15$\,min of kernel time) gives $\gamma = 50\log_2(101195/100000)
= 0.8569\ldots$, and \code{K14.density\_bound} ($1{,}594{,}323$ classes
in $729$ shards across ten files, $\approx 29$\,h of kernel time, peak
$37$\,GB) gives
\[
270437 \cdot 101210^{50y} \cdot 100000^{100} \le \#\{n \le 80000\cdot 2^y
: \text{reaches } 1\}\cdot\big(16777216 \cdot 100000^{50y}\cdot
101210^{100}\big),
\]
i.e.\ $\gamma = 50\log_2(101210/100000) = 0.8676\ldots$, the current
...
Lean's kernel and Mathlib. The $k = 12$ and $k = 13$ modules were additionally replayed through an
independent instance of the kernel by \code{lean4checker} (built for
the same toolchain), which re-evaluates every shard proof from the
stored declarations and reports no discrepancy. What is claimed: exactly the displayed
...
(theorem \code{K8.density\_bound}, superseded by the results of Section~\ref{sec:formal}; its definitional core is factored out as \code{Grid50.lean}):
\[
