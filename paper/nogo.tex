...
preprints, and, as we discuss in Section~\ref{sec:casestudy}, in an
earlier version of this very repository --- runs as follows:
\begin{enumerate}
...
\begin{corollary}[\code{old\_descent\_axiom\_false}]\label{cor:axiom}
The statement ``every $n > 300$ satisfies $C^t(n) < n$ for some
$t \le 53$'' is false: $n = 2^{28} - 1$ rises for its first $56$ Collatz
steps.
\end{corollary}

Corollary~\ref{cor:axiom} is stated in this specific form because it is,
verbatim, the negation of an axiom assumed by an earlier formalization
in this repository (Section~\ref{sec:casestudy}). Empirically the
smallest violator is $n = 447 = 110111111_2$, whose six trailing ones
already exhibit the shadowing mechanism in miniature; there are $1{,}933$
violators below $2 \times 10^5$.
...
\section{Case study: how the false axiom looked plausible}
\label{sec:casestudy}

An earlier version of this repository claimed a conditional proof of the
conjecture from two axioms: verified convergence below $2^{71}$, and the
descent statement refuted in Corollary~\ref{cor:axiom}. The descent
statement was ``verified'' by tracking, for each odd residue class
modulo $2^{14}$, an affine form $c \cdot n + d$ along the orbit, with
worst-case handling of one boundary case. The flaw: the class of $n$
modulo $2^{14}$ determines the orbit's parity decisions only until the
...
ran for up to $53$ steps. Beyond the budget, the tracked trajectory
silently followed the class representative rather than the class. For
...
We record this not to dwell on an error but because it is the
\emph{typical} error of the genre, and because
Theorem~\ref{thm:nogo} explains its inevitability: had the precision
accounting been done correctly at modulus $2^{14}$, the verification
would have failed on the classes near $-1$; enlarging the modulus
relocates but never removes them. By the binomial law
...
in letting failed proof attempts fail \emph{informatively}.

\subsection*{Acknowledgments}
The formalization, the searches, and this text were produced in
collaboration with Claude (Anthropic), including the discovery and
refutation of the repository's earlier incorrect proof. The Lean
community's \textsf{mathlib} made the formalization routine.
