...
measures of a divergent \emph{integer} orbit have full dimension?
\end{question}

An affirmative answer excludes divergent orbits. The question sits
squarely in the joint $\times 2$/$\times 3$ tradition of Furstenberg
\cite{furstenberg} and Rudolph--Johnson \cite{rudolph}: the integers'
base-$2$ and base-$3$ structures are expected to be ``independent,''
and Question~\ref{q:R} asks for one quantitative consequence. We
emphasize the calibration: nothing weaker than dimension
$> 0.9554$ is needed, and the bound $0.95$ from the dynamics leaves a
genuine, fixed gap. We also emphasize honesty about difficulty: even
the much weaker assertion that the binary digits of $3^k$ are not
eventually structured (e.g.\ popcount growth) is open
\cite{senge-straus, stewart}; Question~\ref{q:R} is of that family.
By the no-go theorem of \cite{companion}, no bounded-information
argument resolves it; by \code{parity\_pattern\_realized}, the
compressed shadow sets are inhabited at every finite scale, so no
counting argument alone can either.
